...
SELECT r.region_name AS 'region with most locations'
FROM regions r
JOIN (
  SELECT region_id, COUNT(*) AS count
  FROM countries
...
SELECT COUNT(DISTINCT country_id) AS num_countries_with_locations
FROM locations;
...
SELECT category_name, ROUND(AVG(list_price), 2) AS avg_price
FROM products 
...
SELECT list_price, COUNT(list_price) AS count
FROM products
GROUP BY list_price
ORDER BY count DESC
LIMIT 2;
...
SELECT pc.category_name, p.list_price, COUNT(*) AS count
FROM inventories i
JOIN products p ON i.product_id = p.product_id
JOIN product_categories pc ON p.category_id = pc.category_id
GROUP BY pc.category_name, p.list_price
HAVING count > 1
ORDER BY pc.category_name, count DESC;
...
-- 5a) Find the median of all list_prices.
SET @rowindex := -1;

SELECT AVG(list_price) AS median_price FROM (
  SELECT
...
WHERE rowindex >= total_rows / 2 AND rowindex <= (total_rows + 1) / 2;
